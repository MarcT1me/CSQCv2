...
namespace MirageAPI::DirectX
{
    DX12WindowContext::DX12WindowContext(HWND hwnd, int width, int height)
        : m_rtvHandle0(nullptr), m_rtvHandle1(nullptr), m_width(width), m_height(height)
    {
    ...
    void DX12WindowContext::Resize(int width, int height)
    {
        ...
        m_rtvHandle0->ptr = rtvHandle.ptr;
        rtvHandle.ptr += m_rtvDescriptorSize;
        m_rtvHandle1->ptr = rtvHandle.ptr;

        ID3D12Resource* renderTarget0;
        m_swapChain->GetBuffer(0, IID_PPV_ARGS(&renderTarget0));
        m_renderTarget0 = renderTarget0;
        DX12Context::GetDevice()->CreateRenderTargetView(m_renderTarget0, nullptr, *m_rtvHandle0);

        ID3D12Resource* renderTarget1;
        m_swapChain->GetBuffer(1, IID_PPV_ARGS(&renderTarget1));
        m_renderTarget1 = renderTarget1;
        DX12Context::GetDevice()->CreateRenderTargetView(m_renderTarget1, nullptr, *m_rtvHandle1);
    ...
    void DX12WindowContext::BeginFrame()
    {
        ...
        D3D12_CPU_DESCRIPTOR_HANDLE* currentRtv = m_frameIndex == 0 ? m_rtvHandle0 : m_rtvHandle1;

        ID3D12Resource* currentTarget = m_frameIndex == 0 ? m_renderTarget0 : m_renderTarget1;
    ...
    void DX12WindowContext::EndFrame()
    {
        ...
        DX12Context::GetCommandQueue()->ExecuteCommandLists(1, lists);
    }
