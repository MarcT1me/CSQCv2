...
namespace MirageAPI::DirectX
{
    ...
    void DX12Device::Initialize(DX12DeviceInitFlags flags)
    {
        ...
        UINT factoryFlags = s_isDebug ? DXGI_CREATE_FACTORY_DEBUG : 0;
        CheckHResult(
        ...
        HRESULT hr;
        ID3D12Device* device;
        if (flags.HasFlag(DX12DeviceInitFlags::UseAdapter))
        {
            if (flags.HasFlag(DX12DeviceInitFlags::UseWarpAdapter))
            {
